...
export default function List({
  ...
  function handleKeyDownForList(e) {
    e.stopPropagation();
    if(e.key === "Enter") onRename(boardId, listId);
    if(e.key === "Escape") setIsEditingList(false);
  }

  function onRename(boardId, listId) {
  ...
  function onDelete(boardId, listId){
     useTrelloBoard((boards) => {
      return boards.map(board => {
        if(board.id === boardId){
          return {
            ...board,
            list: [
              ...board.list.filter(l => l.id !== listId)
            ]
          }
        } else{
          return board;
  ...
  return (
    <div
      ...
      ref={setNodeRef}
      {...listeners}
      className="w-60 flex flex-col touch-none cursor-grab active:cursor-grabbing gap-4 font-Poppins shrink-0"
    >
      {/* List header */}
        ...
        {isEditingList ? (
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => onRename(boardId, listId)}
                className="flex items-center justify-center w-7 h-7 rounded-lg
          ...
          <>
            <div className="flex items-center gap-2 min-w-0">
              <h2 className="text-[15px] font-semibold truncate m-0">
                {title}
              </h2>
              <span
                className="text-[12px] text-gray-400 dark:text-gray-600 bg-gray-100 dark:bg-white/10 
            ...
            <Menu as="div" className="relative shrink-0">
              ...
              >
                ...
                <MenuItem>
                  <button
                    onClick={() => onDelete(boardId, listId)}
                    className="flex items-center gap-2 w-full px-3 py-2 text-sm rounded-lg
